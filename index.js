...
const CONFIG = {
  ...
  REQUEST_RETRY_ATTEMPTS: parseInt(process.env.REQUEST_RETRY_ATTEMPTS) || 3,
  REQUEST_RETRY_DELAY_MS: parseInt(process.env.REQUEST_RETRY_DELAY_MS) || 1000,
  PARALLEL_REQUESTS: parseInt(process.env.PARALLEL_REQUESTS) || 5,
...
function checkRateLimit(ip) {
  ...
  if (!rateLimitMap.has(key)) {
    rateLimitMap.set(key, { count: 1, resetTime: now + CONFIG.RATE_LIMIT_WINDOW_MS });
    return true;
  }
  const data = rateLimitMap.get(key);
  if (now > data.resetTime) {
    data.count = 1;
    data.resetTime = now + CONFIG.RATE_LIMIT_WINDOW_MS;
    return true;
  }
    ...
    try {
      ...
      req.on('error', async (err) => {
        if (attempt < CONFIG.REQUEST_RETRY_ATTEMPTS) {
          const delay = CONFIG.REQUEST_RETRY_DELAY_MS * Math.pow(2, attempt - 1);
          Logger.warn(`Retry attempt ${attempt} for ${url} with delay ${delay}ms`);
          await sleep(delay);
    ...
    if (useClaude) {
      if (!ANTHROPIC_KEY) { Logger.warn('ANTHROPIC_KEY not set, skipping Claude news fetch'); }
        ...
        try {
          ...
          Logger.warn('Claude news fetch failed, using RSS fallback', { aihe, error: err.message });
        }
      }
    }
...
${news.map((n, i) => `${i}. "${n.otsikko}" — ${n.kuvaus || ''}`).join('\n')}`;
    ...
    try {
      ...
      const existing = await supabaseGet(`uutiset?aihe=eq.${encodeURIComponent(saveAihe)}&select=url`);
      const existingUrls = new Set(existing.map(item => item.url));
      const newsToSave = news
        .filter(item => item.otsikko && item.url && !existingUrls.has(item.url))
      ...
      if (newsToSave.length > 0) {
        ...
        Logger.info('Saved new news items', { aihe: saveAihe, count: newsToSave.length });
      } else {
        Logger.info('No new news items to save', { aihe: saveAihe });
      }
    } catch (err) {
      Logger.warn('Failed to save news incrementally', { aihe, error: err.message });
    }
  ...
  try {
    if (req.url === '/health') {
      try {
        ...
        if (ANTHROPIC_KEY) {
          try {
            await callClaude([{ role: 'user', content: 'Hello' }], 'Respond with ok', null);
            claudeOk = true;
          } catch (e) { claudeOk = false; }
        }
        res.statusCode = 200;
        res.end(JSON.stringify({ status: 'ok', uptime: process.uptime(), timestamp: new Date().toISOString(), supabase: supabaseOk, claude: claudeOk }));
      } catch (err) {
        res.statusCode = 503;
        res.end(JSON.stringify({ status: 'error', uptime: process.uptime(), timestamp: new Date().toISOString(), error: err.message }));
      }
      return;
    }
    ...
    if (req.url.startsWith('/rss?')) {
      try {
        ...
        const allowedDomains = ['feeds.yle.fi', 'www.ampparit.com', 'news.google.com'];
    ...
    const now = Date.now();
    if (rssCache.has(feedUrl) && (now - rssCache.get(feedUrl).timestamp) < RSS_CACHE_MS) {
      res.setHeader('Content-Type', 'application/xml; charset=iso-8859-1');
      res.end(rssCache.get(feedUrl).data);
      return;
    }
